...
class TaskV3(DatasetProviderBase):
  ...
  def _validate_preprocessing(
      self, dataset: tf.data.Dataset, from_cache:bool =False,
  ) -> tf.data.Dataset:
    """Validates preprocessed dataset, raising Exceptions if needed.

    Args:
      dataset: a tf.data.Dataset to validate.
      from_cache: for backward compatibility, we ignore certain type mismatches
        on cached datasets.
    Returns:
    ...
    for feat, feat_spec in self.output_features.items():
      ...
      if feat_spec.dtype != actual_spec.dtype:
        if (from_cache and feat_spec.dtype == tf.int32 and
            actual_spec.dtype == tf.int64):
          logging.warning(
              "Ignoring dtype mismatch for int32 and int64 on cached dataset."
          )
        else:
          raise ValueError(
  ...
  def get_dataset(
    ...
    ds = self._validate_preprocessing(ds, from_cache=use_cached)
